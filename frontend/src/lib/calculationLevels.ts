...
export interface CalcQuestionType1 {
}
...
export type CalcLevel = {
  ...
  description: string
  maxNumber: number
  generate_problem(maxNumber: number): CalcQuestion
}
...
export const CALC_LEVELS: CalcLevel[] = [
  // ── Level 1: Add pips of 2 dice ────────────────────────────────────────────
  {
    ...
    maxNumber: 10,
    generate_problem(_maxNumber: number): CalcQuestion {
      const val1 = calGame.RandomDice()
      const val2 = calGame.RandomDice()
      const ope = calGame.RandomOperator()
      const [_, result] = calGame.Calculate({ operands: [val1, val2], operators: [ope] })
      return {
        operands: [val1, val2],
        operators: [ope],
        result: result,
