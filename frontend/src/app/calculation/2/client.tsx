...
export default function ClientPage(props: Props) {
  ...
  const handleClick = function(answer: number) {
    const [_,result] = game.Calculate({ answer: answer, operands: [val1, val2], operators: [operator] })
    setAnswerResult(result)

    // สุ่มโจทย์ใหม่และรีเซ็ตคำตอบ
    setTimeout(() => {
      const [newVal1, newVal2] = [game.RandomValue(props.maxNumber), game.RandomValue(props.maxNumber)]
